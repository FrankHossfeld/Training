...
public class PersonResource {
  ...
  public PersonInsertResponse insert(Person person) {
    Iterator<Person> iter = dummyDB.values().iterator();
    long maxKey = 0;
    while (iter.hasNext()) {
    ...
    maxKey++;
    person.setId(maxKey);
    dummyDB.put(new Long(maxKey), person);
  ...
  public PersonUpdateResponse update(Person person) {
    ...
    if (value != null) {
      ...
      dummyDB.put(new Long(person.getId()), person);
    }
  ...
  public PersonSearchResponse search(PersonSearch search) {
    ...
    if ((search.getName() != null && search.getName().length() != 0) || (search.getCity() != null && search.getCity()
        .length() != 0)) {
      Iterator<Long> iterator = dummyDB.keySet().iterator();
      while (iterator.hasNext()) {
        Person Person = dummyDB.get(iterator.next());
        if (search.getName() != null && search.getName().length() != 0 && search.getCity() != null && search.getCity()
            .length() != 0) {
          if (Person.getName().toLowerCase().contains(search.getName().toLowerCase()) && Person.getAddress().getCity()
              .toLowerCase().contains(search.getCity().toLowerCase())) {
            list.add(Person);
          }
  ...
  private void init() {
    ...
    dummyDB.put(new Long(1), new Person(1, "Simpsons", "Homer", address01));
    dummyDB.put(new Long(2), new Person(2, "Simpsons", "Marge", address01));
    dummyDB.put(new Long(3), new Person(3, "Simpsons", "Bart", address01));
    dummyDB.put(new Long(4), new Person(4, "Simpsons", "Maggie", address01));
    dummyDB.put(new Long(5), new Person(5, "Simpsons", "Lisa", address01));
    Address address02 = new Address(2, "Blumenweg Nr. 13", "88999", "Entenhausen");
    dummyDB.put(new Long(6), new Person(6, "Duck", "Donald", address02));
    dummyDB.put(new Long(7), new Person(7, "Duck", "Trick", address02));
    dummyDB.put(new Long(8), new Person(8, "Duck", "Tick", address02));
    dummyDB.put(new Long(9), new Person(9, "Duck", "Tack", address02));
    Address address03 = new Address(2, "Am Goldberg Nr. 1", "88991", "Entenhausen");
    dummyDB.put(new Long(10), new Person(10, "Duck", "Dagobert", address03));
